...
class AccurateSolarSensorDB:
    ...
    async def async_load(self):
        ...
        if data is None:
            ...
            # Clean up old "default" roofs from DB if they somehow existed
            keys_to_delete = [
                k for k, v in self.roofs.items() 
                if getattr(v, "get", lambda x: None)("name", "").lower() in ["default", "default roof"]
                or k in ["default", "default_roof"]
            ]
            if keys_to_delete:
    ...
    def add_sensor_group(self, name, irradiance_sensor, temp_sensor, temp_panel_sensor, wind_sensor, ref_tilt, ref_orientation, weather_entity=None, illuminance_sensor=None):
